...
DEFAULT_CSV = 'your_merged_historical_dataset.csv'
...
COLUMN_MAPPING = {
    'R_odds': 'RedOdds',
    'B_odds': 'BlueOdds',
    # Add your mappings here as needed
}
...
if df is None:
    st.error(f"No data file found. Please upload a CSV file in the sidebar.")
    st.stop()

# --- CHECK FOR NEEDED COLUMNS ---
missing = [c for c in FEATURE_COLS if c not in df.columns]
if missing:
    st.error(f"Missing columns in data after mapping: {missing}\n\n"
             "If your file uses different names, update COLUMN_MAPPING in the code.")
...
for i, row in df.iterrows():
    if row['red_value'] > VALUE_EDGE:
        value_bets.append({'fight': f"{row.get('r_fighter','')} vs {row.get('b_fighter','')}", 'side': 'Red', 'odds': row['RedOdds'],
                           'prob': row['model_red_prob'], 'edge': row['red_value'], 'winner': row.get('winner', '')})
    if row['blue_value'] > VALUE_EDGE:
        value_bets.append({'fight': f"{row.get('r_fighter','')} vs {row.get('b_fighter','')}", 'side': 'Blue', 'odds': row['BlueOdds'],
                           'prob': row['model_blue_prob'], 'edge': row['blue_value'], 'winner': row.get('winner', '')})
...
if value_bets_df.empty:
    ...
    for _, row in value_bets_df.iterrows():
        if row['side'] == 'Red':
            won = (str(row['winner']).lower() == 'red')
            odds = row['odds']
        else:
            won = (str(row['winner']).lower() == 'blue')
            odds = row['odds']
