...
uploaded_file = '/mnt/data/your_merged_historical_dataset (2).csv'

if not os.path.exists(uploaded_file):
    st.warning("Please upload your merged historical dataset (CSV) to use this feature.")
else:
    hist_df = pd.read_csv(uploaded_file)

    # -- Try to infer key columns, you can adjust here:
    # Typical columns: 'RedFighter','BlueFighter','RedOdds','BlueOdds','Winner','RedProb','BlueProb'
    ...
    if missing:
        ...
        # If you don't have model probabilities, you can use real fight outcome as a simple model,
        # or predict with your model (if you have features in the CSV).
        if 'RedProb' not in hist_df.columns or 'BlueProb' not in hist_df.columns:
            # Example: assign random or neutral probabilities (not recommended)
        ...
        for idx, row in hist_df.iterrows():
            ...
            if row['RedEdge'] > sim_edge_thresh:
                ...
                won = (row['Winner'].lower() == "red") or (row['Winner'] == row['RedFighter'])
                sim_bets.append({
                ...
                won = (row['Winner'].lower() == "blue") or (row['Winner'] == row['BlueFighter'])
                sim_bets.append({
                    'Fight': f"{row['RedFighter']} vs {row['BlueFighter']}",
