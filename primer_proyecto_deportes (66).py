...
def simulate_bankroll(bets, stake=1.0, starting_bankroll=10000):
    ...
    for i, row in bets.iterrows():
        if row['BetOn'] == 'Red':
            odds = row['RedOdds']
            won = (str(row.get('winner', '')).lower() == 'red')
        else:
            odds = row['BlueOdds']
            won = (str(row.get('winner', '')).lower() == 'blue')
        if odds <= 0 or np.isnan(odds): continue
        if won:
...
if uploaded_file and model_file:
    ...
    st.dataframe(value_bets if len(value_bets) < 100 else value_bets.head(100))  # Show up to 100 bets
