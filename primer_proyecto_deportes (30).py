...
def main():
    ...
    for i, vb in enumerate(value_bets, start=1):
        # Determine which fraction to stake
        ...
        # figure out if that bet won:
        won = (vb['Side'] == vb['Side'] and  # this line just confirms the side
               any(
                   ((df_hist.loc[idx, 'RedFighter'] == vb['Fighter']) and vb['Side']=='Red' and df_hist.loc[idx,'Winner']=='Red') or
                   ((df_hist.loc[idx, 'BlueFighter'] == vb['Fighter']) and vb['Side']=='Blue' and df_hist.loc[idx,'Winner']=='Blue')
    ...
    # Turn it into a DataFrame and show it
    sim_df = pd.DataFrame(sim_rows)
    st.dataframe(sim_df)

    # Plot the balance curve
    st.line_chart(sim_df.set_index('Bet #')['Balance ($)'])

    # And the summary
    st.write(f"**Initial Bankroll:** ${INITIAL_BANKROLL:,.2f}")
    st.write(f"**Final Bankroll:**   ${bankroll:,.2f}")
    st.write(f"**ROI:**              {((bankroll-INITIAL_BANKROLL)/INITIAL_BANKROLL*100):.1f}%")